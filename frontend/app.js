...
// Membaca URL backend dari localStorage (jika diset oleh user untuk Google Colab), default ke localhost
let API_BASE = localStorage.getItem("colab_api_url") || "http://localhost:8000";

// ============================================================
// GOOGLE COLAB & API CONFIGURATION UTILITIES
// ============================================================
function toggleColabSettings() {
    const panel = document.getElementById("colab-settings-panel");
    if (panel) {
        if (panel.style.display === "none" || panel.style.display === "") {
            panel.style.display = "block";
            // Pre-fill input with current value if exists
            const input = document.getElementById("colab-url-input");
            if (input) {
                input.value = localStorage.getItem("colab_api_url") || "";
            }
...
function saveColabUrl() {
    ...
    if (!url) {
        alert("⚠️ Masukkan URL terlebih dahulu!");
        return;
    }
    ...
    alert("✅ URL Colab berhasil disimpan! Menghubungkan ulang...");
    if (document.getElementById("colab-settings-panel")) {
        document.getElementById("colab-settings-panel").style.display = "none";
    }
    
    // Cek koneksi ulang ke server baru
    checkServerStatus();
    loadGallery();
}

function resetColabUrl() {
    ...
    alert("🔄 Koneksi di-reset ke Local Server (localhost:8000)");
    if (document.getElementById("colab-settings-panel")) {
        document.getElementById("colab-settings-panel").style.display = "none";
    }
...
// Daftarkan fungsi ke window agar bisa diakses oleh HTML onclick
window.toggleColabSettings = toggleColabSettings;
window.saveColabUrl = saveColabUrl;
window.resetColabUrl = resetColabUrl;
...
async function generateVideo() {
    ...
    const requestBody = {
        ...
        num_frames: document.getElementById("num-frames") ? parseInt(document.getElementById("num-frames").value) : 33,
...
function showResult(data) {
    ...
    // Auto-Sync ke Folder Lokal (F:\...\outputs) melalui Server Lokal
    if (API_BASE !== "http://localhost:8000") {
        fetch("http://localhost:8000/sync_video", {
            method: "POST",
            ...
            if (res.ok) console.log("✅ Proses Sinkronisasi ke Lokal Dimulai!");
        }).catch(err => console.log("Gagal sinkronisasi ke lokal:", err));
    }
...
async function loadGallery() {
    ...
    try {
        // SELALU ambil dari server LOKAL untuk Galeri (Mengakses F:\...\outputs)
        ...
        gallery.innerHTML = data.videos.map(v => {
            ...
            <div class="gallery-item" id="item-${v.filename}">
                <!-- SELALU mainkan dari folder LOKAL, loop dihilangkan -->
                <video src="http://localhost:8000${v.url}" controls muted preload="metadata"></video>
...
async function deleteVideo(filename) {
    ...
    try {
        // 1. Hapus dari Server Lokal (F:\...\outputs)
        const resLocal = await fetch(`http://localhost:8000/videos/${filename}`, { 
            method: "DELETE" 
        });

        // 2. Hapus dari Cloud/Kaggle jika tersambung via Ngrok
        if (API_BASE !== "http://localhost:8000") {
            await fetch(`${API_BASE}/videos/${filename}`, { 
                method: "DELETE",
                headers: { "ngrok-skip-browser-warning": "true" }
            }).catch(e => console.log("Gagal hapus dari cloud, abaikan."));
        }
...
async function unloadModel() {
    ...
    try {
        const res = await fetch(`${API_BASE}/model/unload`, { 
            method: "POST",
            headers: {
                "ngrok-skip-browser-warning": "true"
            }
