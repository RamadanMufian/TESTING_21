...
class VideoGeneratorModel:
    ...
    Mendukung Text-to-Video & Image-to-Video.
    """
    ...
    def load_model(self):
        ...
        print("[Model] Loading LTX Video (T2V & I2V)... (bisa 2-5 menit pertama kali)")
        self._clear_vram()

        try:
            # Load T2V pipeline
            self.t2v_pipe = LTXPipeline.from_pretrained(
                MODEL_ID,
            ...
            # Load I2V pipeline
            self.i2v_pipe = LTXImageToVideoPipeline(**self.t2v_pipe.components)
            ...
            self.t2v_pipe.enable_attention_slicing(slice_size="auto")

            self.i2v_pipe.enable_model_cpu_offload()
            self.i2v_pipe.vae.enable_slicing()
            self.i2v_pipe.vae.enable_tiling()
            self.i2v_pipe.enable_attention_slicing(slice_size="auto")
            # ==========================================

            self.is_loaded = True
            print(f"[Model] ✅ Model T2V & I2V berhasil di-load tanpa ganda VRAM!")
    ...
    def generate_video(
        ...
        """
        Generate video dari text prompt (atau image).
        Returns:
        ...
        width = (min(width, 512) // 32) * 32
        height = (min(height, 512) // 32) * 32
        num_frames = (min(num_frames, 1500) // 8) * 8 + 1  # format 8n+1
        ...
        print(f"[Generate] Seed: {seed}, I2V: {is_i2v}")
