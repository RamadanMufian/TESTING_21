...
class GenerateRequest(BaseModel):
    ...
    height: int = Field(default=256, ge=64, le=480)
    num_frames: int = Field(default=33, ge=8, le=1500, description="Jumlah frame (8-1500)")
...
class GenerateI2VRequest(BaseModel):
    image_b64: str = Field(...)
    prompt: str = Field(..., min_length=3, max_length=500, description="Deskripsi video")
    negative_prompt: str = Field(default="worst quality, inconsistent, blurry, noisy, distorted, artifact, low resolution")
    width: int = Field(default=480, ge=64, le=640)
    height: int = Field(default=256, ge=64, le=480)
    num_frames: int = Field(default=33, ge=8, le=1500)
    num_inference_steps: int = Field(default=40, ge=4, le=100)
    guidance_scale: float = Field(default=3.5, ge=1.0, le=10.0)
...
@app.post("/generate_i2v", response_model=GenerateResponse)
async def generate_video_i2v(request: GenerateI2VRequest, background_tasks: BackgroundTasks):
    if generation_queue.full():
        raise HTTPException(status_code=429, detail="Antrian penuh. Coba lagi dalam beberapa menit.")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Format gambar invalid: {e}")
    ...
    background_tasks.add_task(_run_generation, job_id=job_id, request=req, is_i2v=True, img=img)
    return GenerateResponse(success=True, job_id=job_id)

from fastapi import UploadFile, Form, File
import shutil

@app.post("/enhance_video", response_model=GenerateResponse)
async def enhance_video_endpoint(
    background_tasks: BackgroundTasks,
    video_file: UploadFile = File(...),
    upscaler: str = Form("2x"),
    face_restore: str = Form("none"),
    interpolation: str = Form("none"),
    prompt: str = Form(""),
    num_frames: int = Form(33),
    steps: int = Form(40),
    cfg_scale: float = Form(3.5),
    seed: int = Form(-1)
):
    if generation_queue.full():
    ...
    job_id = str(uuid.uuid4())[:12]
    temp_path = OUTPUT_DIR / f"temp_upload_{job_id}.mp4"
    with open(temp_path, "wb") as buffer:
        shutil.copyfileobj(video_file.file, buffer)
        
    job_status[job_id] = {"status": "queued", "result": None}
    
    # Untuk server lokal, kita bypass saja karena ini akan dikirim ke Kaggle jika menggunakan Kaggle.
    # Jika berjalan di lokal murni, ini akan memanggil fungsi dummy/passthrough.
    background_tasks.add_task(_run_enhancement_local, job_id=job_id, filepath=temp_path, upscaler=upscaler, face_restore=face_restore, interpolation=interpolation, prompt=prompt, num_frames=num_frames, steps=steps, cfg_scale=cfg_scale, seed=seed)
    
    return GenerateResponse(success=True, job_id=job_id)

async def _run_enhancement_local(job_id: str, filepath: Path, upscaler: str, face_restore: str, interpolation: str, prompt: str, num_frames: int, steps: int, cfg_scale: float, seed: int):
    job_status[job_id]["status"] = "processing"
    # Simulasi proses (karena lokal tidak sanggup menjalankan pipeline 30GB)
    await asyncio.sleep(3)
    
    # Hanya rename file sebagai bukti berhasil
    out_name = f"enhanced_{job_id}.mp4"
...
async def _run_generation(job_id: str, request: GenerateRequest, is_i2v=False, img=None):
...
@app.post("/sync_video")
async def sync_video(req: SyncRequest, background_tasks: BackgroundTasks):
    """Menerima request dari Web UI untuk mendownload hasil Colab langsung ke folder outputs lokal."""
    background_tasks.add_task(_download_sync, req.video_url, req.filename, req.json_metadata)
    return {"message": "Auto-Syncing dimulai"}

def _download_sync(video_url: str, filename: str, json_metadata: dict):
    import urllib.request
    import json
    try:
        print(f"[Auto-Sync] Mengunduh video dari Colab: {filename}...")
        req = urllib.request.Request(video_url, headers={'ngrok-skip-browser-warning': 'true'})
        ...
        if json_metadata:
            with open(target_path.with_suffix(".json"), "w", encoding="utf-8") as jf:
                json.dump(json_metadata, jf)
                
        print(f"[Auto-Sync] ✅ Berhasil tersimpan di: {target_path}")
    except Exception as e:
        print(f"[Auto-Sync] ❌ Gagal mengunduh video: {e}")
